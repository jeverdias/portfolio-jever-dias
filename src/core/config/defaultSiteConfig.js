import { defaultAppearance } from '../../data/appearance.js'
import { defaultSiteClassification, siteClassifications } from '../../data/siteClassifications.js'
import { cloneConfigValue, deepFreezeConfig } from './siteSchema.js'

const portfolioClassification = siteClassifications.find((item) => item.id === defaultSiteClassification)

const canonicalDefaults = {
  classificationLayoutVersion: 3,
  siteClassificationId: defaultSiteClassification,
  siteClassification: portfolioClassification?.classification,
  siteClassificationDescription: portfolioClassification?.goal,
  name: 'Jever Dias',
  role: 'BI Developer | Analytics | Sistemas Web Básicos',
  eyebrow: 'BI · Dados · Sistemas · Impacto',
  intro: 'Transformo dados em dashboards, insights e sistemas simples para apoiar decisões e melhorar processos.',
...
export const defaultSiteConfig = deepFreezeConfig(cloneConfigValue(canonicalDefaults))
export const createDefaultSiteConfig = () => cloneConfigValue(defaultSiteConfig)
export const technologies = deepFreezeConfig(['Power BI', 'DAX', 'JavaScript', 'Supabase', 'GPTs', 'Agentes de IA'])
